"""
tasting_retrieval_equa_chem_v5.0_piette_RM.py
===============================================
Series 005PM-EQ — Absolute flux + physical (R/d)² scaling

[004PM-EQ → 005PM-EQ: Guidebook v1.1 → v2.0]
  ...
  - log_R prior changed to uniform U(-0.1, 0.5) — the absolute flux level
    constrains R directly; evolutionary model prior not needed.
  - log_M prior remains Gaussian N(1.079, 0.145) from Xuan+2024 Table 1.
  - Distance: D_DH_TAU_PC = 135.2 pc (Gaia DR3).

Purpose: test whether the SINFONI-calibrated flux constrains R independently
of evolutionary models, providing a physical anchor for log_g.

Start with N=100 pilot to verify:
  1. log_R is constrained by data (posterior narrower than prior)
  2. chi² per pixel is comparable to normalised runs (~1.5)
  3. Flux level matches SINFONI calibration within ~10%
...
Free params: C_H ([C/H]), C/O, log_12CO_13CO, log_M, log_R
  log_g derived internally: g = G*M/R²
  [Z/H] derived internally: [Z/H] = [C/H] − log10(C/O / 0.5495)
...
N_points     = 800           # start with pilot run
evidence_tol = 0.5
...
# ===========================================================================
# FREE PARAMETERS — 005PM-EQ (absolute flux + M+R)
# ===========================================================================
...
# Range U(-0.1, 0.5) = 0.79–3.16 RJup (wide; data should narrow this).
# Already set in make_free_params_equilibrium() for v2.0; shown here for clarity.
# free_params['log_R'] = ([-0.1, 0.5], r'$\log R/R_{\rm Jup}$')

# log_M: Gaussian from Xuan+2024 Table 1 (DH Tau b: 12±4 MJup) — already set.
...
plt.title('P-T profile (Piette+2020 PCHIP) — 005PM-EQ absolute flux')
