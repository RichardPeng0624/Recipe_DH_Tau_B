"""
Guidebook_GAStronomy_Piette_v4.2.5.py
=====================================
Series 005PM — Absolute flux mode: (R/d)² physical scaling  [updated from v2.0]

Changes from v4.2 (v4.2.5)
--------------------------
v4.2.5 patch (2026-07-20) — Na and Ca opacity restored to the model spectrum,
ZERO new free parameters (follow-up to the 2026-07-07 removal, which tested
removing Na/Ca as *sampled parameters*; this patch tests removing them as
*opacity sources* while keeping the same 19/24-parameter count as the
3497146/3708914 benchmark pair).
  - Verified directly against the installed pRT3 table
    (petitRADTRANS.chemistry.utils._solar_elemental_abundances /
    PreCalculatedEquilibriumChemistryTable().species, Lodders 2020): Na IS a
    native equilibrium-chemistry-table species; Ca (and Ti) are NOT. The old
    equilibrium_chemistry() docstring claim that "HF, Na, Ca, Ti are absent
    from the equilibrium table" was WRONG for Na — corrected below.
  - EQ_SPECIES_PRT3 / EQ_LABEL_NAMES: '23Na' and '40Ca' UNCOMMENTED ('48Ti'
    stays commented — out of scope for this patch, unrelated v4.0 decision).
  - Na: no new code needed. Because '23Na' simplifies to 'Na'
    (petitRADTRANS.chemistry.utils.simplify_species_list) and 'Na' is a
    native table key, the existing `elif simple in mf_eq` branch in
    equilibrium_chemistry() picks it up automatically, driven by the
    already-sampled C_H/C/O free params — same mechanism as H2O/CO/CH4/etc.
  - Ca: new LOG_CA_H_SOLAR = -5.73 constant (Lodders 2020: A(Ca) = 6.27,
    atomic number 20, from petitRADTRANS's own
    _solar_elemental_abundances table — same source already backing this
    file's CO_SOLAR = 0.5495) and a new metallicity-scaled injection block in
    equilibrium_chemistry(): VMR_Ca = 10**(LOG_CA_H_SOLAR + ZH) x 2 x VMR_H2,
    mirroring the existing HF block's H2-normalisation convention but keyed
    to ZH (already derived internally from C_H/C/O) instead of a dedicated
    free parameter like HF's F_H. No log_Ca parameter is read or required.
  - _atomic_log dict (free-VMR injection for atoms not in the eq table):
    '23Na' entry REMOVED (Na now sourced from the table branch above, not a
    free param — leaving the dict entry would have been dead/misleading code
    since log_Na was never restored to any free-param function). '40Ca'
    entry REMOVED (superseded by the new ZH-scaled block; log_Ca likewise
    stays absent from every free-param function). '48Ti' entry left as-is
    (inert — Ti stays out of EQ_SPECIES_PRT3).
  - make_free_params_equilibrium() / make_free_params_free_chem(): UNCHANGED
    — log_Na and log_Ca remain absent. This is the entire point of the
    patch: parameter count for both cloud-free and cloud configs stays
    identical to the 3497146/3708914 benchmark (19/24).
  - Full derivation, verification commands, and the 6-step rollout plan:
    recording_recipe.md, 2026-07-20 "Investigation: can Na/Ca opacity
    re-enter the model spectrum WITHOUT adding free parameters?" and the
    execution log entry below it.
...
EQ_SPECIES_PRT3 = [
    ...
    '23Na',               # Na   — v4.2.5: native eq-table species; driven by C_H/C/O, NOT a free param
    # '48Ti',             # Ti   — commented out (not in eq table; out of scope for v4.2.5)
    '40Ca',               # Ca   — v4.2.5: solar-scaled by ZH (see LOG_CA_H_SOLAR); NOT a free param
]
...
# v4.2.5: solar calcium reference for the metallicity-scaled Ca injection.
# log10(n_Ca/n_H)_solar = A(Ca) - 12 = 6.27 - 12 = -5.73 (Lodders 2020,
# atomic number 20), read directly from petitRADTRANS's own
# _solar_elemental_abundances table (chemistry/utils.py) — the same Lodders
# 2020 source already backing this file's CO_SOLAR = 0.5495, so no new
# solar-reference inconsistency is introduced.
LOG_CA_H_SOLAR = -5.73
...
class pRT_spectrum:
    ...
    def equilibrium_chemistry(self, params):
        ...
        HF, Ti are absent from the equilibrium table; Na IS a native table
        species (verified directly against the installed pRT3 table, Lodders
        2020 — this corrects a v4.0-v4.2 docstring error that claimed Na was
        also absent). Ca is absent from the table (correctly, as before).

        HF uses the [F/H] parameterisation (Picos+2024 Eq. 3): the free param
        F_H gives [F/H] and VMR_HF is derived as
        10^([F/H] + LOG_F_H_SOLAR + log10(2 × VMR_H2)).

        Na (v4.2.5): resolved automatically by the main table-lookup loop
        below (simplify_species_list('23Na') == 'Na', a native mf_eq key) —
        driven entirely by the existing C_H/C/O free params, exactly like
        H2O/CO/CH4/etc. No dedicated free parameter is read.

        Ca (v4.2.5): NOT in the table, so it is instead scaled directly from
        the derived metallicity ZH using a fixed solar Ca/H ratio (Lodders
        2020): VMR_Ca = 10^(LOG_CA_H_SOLAR + ZH) × 2 × VMR_H2 — same
        H2-normalisation convention as the HF block, but keyed to ZH (already
        computed above from C_H/C/O) rather than to a dedicated free
        parameter. No log_Ca parameter is read or required.

        Ti remains fully out of scope (commented out of EQ_SPECIES_PRT3,
        unchanged since the v4.0→v4.2 decision to drop it).

        Reference
        ---------
        Lodders, K. (2020). Solar elemental abundances. (Na table membership,
          A(Ca) = 6.27 → LOG_CA_H_SOLAR = -5.73, and CO_SOLAR = 0.5495 above
          all read from petitRADTRANS's own solar-abundance table, the same
          source, to avoid a mixed-reference inconsistency.)
        ...
        for prt_name, simple in zip(self.species, simple_names):
            if prt_name == '13C-16O':
                ...
                pass  # v4.2.5: not in eq table; injected below from ZH, see LOG_CA_H_SOLAR block
        ...
        # v4.2.5: Ca — solar-abundance-scaled by the derived metallicity ZH,
        # NOT a free parameter (log_Ca is not read; contrast with the pre-v4.2
        # treatment below via _atomic_log, which required log_Ca in self.params).
        # [Ca/H] is assumed to track [Z/H] exactly (same assumption implicit
        # in every other element the eq-chem table itself scales by ZH) —
        # VMR_Ca = 10^(LOG_CA_H_SOLAR + ZH) x 2 x VMR_H2 (same H2-normalisation
        # convention as the HF block above).
        if '40Ca' in self.species:
            _mean_MMW  = float(np.mean(MMW))
            _vmr_h2    = float(np.mean(mf_eq['H2'])) * _mean_MMW / 2.016
            _log_vmr_ca = LOG_CA_H_SOLAR + ZH + np.log10(2.0 * max(_vmr_h2, 1e-10))
            _vmr_ca    = 10 ** _log_vmr_ca * np.ones(self.n_atm_layers)
            mass_frac['40Ca'] = 40.078 * _vmr_ca / _mean_MMW

        # Ti: direct log VMR free parameter (pre-v4.2 treatment, retained only
        # for Ti — inert here since '48Ti' stays out of EQ_SPECIES_PRT3 and
        # log_Ti is not a free param in any current config).
        _atomic_log = {
            '48Ti': ('log_Ti', 47.867),
        }
...
def make_free_params_free_chem():
    ...
    return {
        ...
        'T_anchor': ([1500, 3500], r'$T_{\rm anchor}$'),   # at 0.2 bar; Table C1: U(1800,3000)
        'dT_1':     ([50, 1000], r'$\Delta T_1$'),  # +0.7 →  0.0 bar (0.7 dex); Table C1: U(200,900)
        'dT_2':     ([  0,  600], r'$\Delta T_2$'),  #  0.0 → -0.3 bar (0.3 dex); Table C1: U(0,450)
        'dT_3':     ([  0,  500], r'$\Delta T_3$'),  # -0.3 → -0.7 bar (0.4 dex); Table C1: U(50,550)
        'dT_4':     ([  0,  500], r'$\Delta T_4$'),  # -0.7 → -1.0 bar (0.3 dex); Table C1: U(0,450)
        'dT_5':     ([ 50,  700], r'$\Delta T_5$'),  # -1.0 → -1.5 bar (0.5 dex); Table C1: U(50,650)
        'dT_6':     ([  0,  800], r'$\Delta T_6$'),  # -1.5 → -3.0 bar (1.5 dex); Table C1: U(200,900)
        'dT_7':     ([  0,  800], r'$\Delta T_7$'),  # -3.0 → -5.0 bar (2.0 dex); Table C1: U(100,700)
