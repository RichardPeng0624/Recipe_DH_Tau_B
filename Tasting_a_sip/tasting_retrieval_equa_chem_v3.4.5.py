...
class Parameters:
    ...
    def __call__(self, cube, ndim=None, nparams=None):
        ...
        for i, key_i in enumerate(self.param_keys):
            ...
            if key_i in ['nabla_3','nabla_5']:  # radiative zone: lower bound 0.00
...
class pRT_spectrum:
    ...
    def __init__(self,
        ...
        #self.mass_fractions, self.CO, self.C_H = self.free_chemistry(self.species,self.params)
        if self.params.get('chemistry', 'free') == 'free':
            self.mass_fractions, self.CO, self.C_H = self.free_chemistry(
                self.species, self.params
            )
        elif self.params['chemistry'] == 'equilibrium':
            self.mass_fractions, self.CO, self.C_H = self.equilibrium_chemistry(
                self.params
            )
        else:
    ...
    def free_chemistry(self,line_species,params):
        ...
        for species_i in species_info.index:
            ...
            line_species_i = PRT3_SPECIES_OVERRIDES.get(line_species_i, line_species_i)
        ...
        CO = C/O
        log_CH_solar = -3.54  # log10(n_C/n_H)_solar, Asplund et al. (2021)
        # [C/H] = log10(n_C/n_H) − log10(n_C/n_H)_solar  (Pico et al. 2025, GQ Lup b)
    ...
    def equilibrium_chemistry(self, params):
        ...
        Species abundances are a function of T, P, C/O, [Z/H].

        v3.4.5: the metallicity free parameter is now [C/H] instead of [Fe/H]/[Z/H].
        The table's [Z/H] axis is derived internally from [C/H] and C/O via:

            [Z/H] = [C/H] − log10(C/O / (C/O)_solar)

        where (C/O)_solar = 0.5495 (Lodders 2020, as used in pRT3).

        Physical motivation: the K-band directly traces CO and CH4, making [C/H]
        more directly constrained than [Fe/H].  At the retrieved C/O = 0.31,
        [C/H] ≈ [Fe/H] − 0.25 dex.

        13CO is absent from the table and is derived from 12CO using the solar
        12C/13C isotope ratio of 70 (Asplund et al. 2021).
        """
        C_H = params['C_H']   # [C/H]: log10(C/H) − log10(C/H)_solar  (solar = 0.0)
        CO  = params['C/O']   # C/O ratio (solar ≈ 0.55; table range: 0.1 to 1.6)

        # Convert [C/H] → [Z/H] for the table.
        # At fixed C/O, a change in [C/H] implies an equal change in all metals:
        #   O/H = C/H / C/O  →  [Z/H] ≈ [O/H] = [C/H] − log10(C/O / (C/O)_solar)
        CO_SOLAR = 0.5495   # Lodders 2020 (matches pRT3 internal solar abundances)
        ZH = C_H - np.log10(CO / CO_SOLAR)   # [Z/H] passed to the table
    ...
    def make_spectrum(self, data_wave=None, out_res=100_000, rv_key='rv_N1'):
        ...
        out_res   : int
            ...
            In v3.4 this is estimated per-night from the wavelength solution and
            passed in by the caller (PMN_lnL / get_params_and_spectrum).
        rv_key    : str
    ...
    def make_pt(self):
        ...
        # v3.4.5: nabla_4 (2nd radiative node) reuses nabla_5 — no new free parameter,
        # Updated 04-07: attempt: remove nabla_5 but keep nabla_4 at log_P_RCE − dlog_P_top
        # but restores the intermediate interpolation node at log_P_RCE − 2·dlog_P_top
        # for a smoother transition between the radiative zone and the model top.
        log_P_nodes = np.array([
             ...
             log_P_RCE -   dlog_P_top,        # just above RCE, 1st radiative node
             #log_P_RCE - 2*dlog_P_top,        # 2nd radiative node (nabla_4, reuses nabla_5)
        ...
        nabla_nodes = np.array([
            ...
            self.params['nabla_3'],
            self.params['nabla_4'],   # nabla_4: reach the top of atmosphere
            #self.params['nabla_5'],   # nabla_5: top of atmosphere
        ])
...
class Retrieval:
    ...
    def __init__(self, parameters, N_live_points, evidence_tolerance, targets,
        ...
        instrument_res : CRIRES+ resolving power R = λ/Δλ used for LSF broadening.
                         ...
                         Default: 100 000.  In v3.4 this is estimated per-night from
                         the wavelength solution and passed in from outside.
    ...
    def PMN_lnL(self, cube=None, ndim=None, nparams=None):
        ...
        # Night 1 — broaden to night-1 instrument resolution, apply rv_N1
        flux1 = self.model_object.make_spectrum(
            data_wave=self.data_wave, out_res=self.instrument_res[0], rv_key='rv_N1'
        )
        ln_L = self.LogLike(flux1, self.Cov)

        # Night 2 — re-use cached pRT flux, broaden to night-2 resolution, apply rv_N2
        if self.two_night_mode:
            flux2 = self.model_object.make_spectrum(
                data_wave=self.data_wave2, out_res=self.instrument_res[1], rv_key='rv_N2'
            )
    ...
    def get_params_and_spectrum(self):
        ...
        self.params_dict['[C/H]_xsolar'] = 10**self.model_object.C_H   # linear ×solar (Xuan+2024 convention)
...
# v3.4: always uses native (topocentric) FITS wavelength solution.
# Instrument resolving power is estimated per-night from the wavelength solution.
# -----------------------------------------------------------------------
def _load_night(night, flux_file, err_file, normalize_method, n_orders=5, n_dets=3):
    ...
    R_est : float — estimated instrument resolving power from wavelength solution
    """
...
# -------------------------------------------------------#
# --- FREE CHEMISTRY (disabled in equa_chem version) ----#
# -------------------------------------------------------#
# constant_params = {}
# free_params = {
#     'rv': ({'type': 'gaussian', 'mu': 16.3, 'sigma': 0.5}, r'$v_{\rm rad}$'),
...
# -------------------------------------------------------#
# --- EQUILIBRIUM CHEMISTRY (active) --------------------#
# -------------------------------------------------------#
# Species list fixed in get_species() for K-band:
...
free_params = {
    ...
    'rv_N1':   ({'type': 'gaussian', 'mu': 32.0, 'sigma': 0.5}, r'$v_{\rm rad,\,N1}$'),
    'rv_N2':   ({'type': 'gaussian', 'mu': 32.0, 'sigma': 0.5}, r'$v_{\rm rad,\,N2}$'),
    'vsini':   ([0, 20],                                          r'$v$ sin$i$'),
    'epsilon': ([0.0, 1.0],                                       r'$\epsilon_{\rm LD}$'),
    'log_g':   ({'type': 'gaussian', 'mu': 3.7, 'sigma': 0.2},   r'log $g$'),
    'nabla_RCE':  ([0.04, 0.34],  r'$\nabla_{T,\rm RCE}$'),
    ...
    'nabla_2':    ([0.04, 0.34],  r'$\nabla_{T,2}$'),
    'nabla_3':    ([0.00, 0.34],  r'$\nabla_{T,3}$'),
    # nabla_4 reuses nabla_5 (v3.4.5): node at log_P_RCE − 2·dlog_P_top restored for
    # smoother interpolation; gradient tied to nabla_5 so no new free parameter is added.
    'nabla_4':    ([0.00, 0.34],  r'$\nabla_{T,4}$'),
    #'nabla_5':    ([0.00, 0.34],  r'$\nabla_{T,5}$'), #stop using nabla_5 as a free parameter to reduce degeneracy with nabla_4; now tied to nabla_4 so no new free parameter is added
    'T_bottom':   ([2000, 5000],  r'$T_{\rm bot}$'),
    # log_P_RCE: Gaussian prior centred on 4131995 result (−0.93); σ=0.30 is wider
    # than the posterior (±0.25/0.18) to remain conservative and allow exploration.
    #'log_P_RCE':  ({'type': 'gaussian', 'mu': -0.93, 'sigma': 0.30}, r'log $P_{\rm RCE}$'),
    'log_P_RCE':  ([-3.0,  1.0],  r'log $P_{\rm RCE}$'),
    # dlog_P_bot: upper bound tightened from 1.60 → 1.40; 18% of 4131995 samples
    # exceeded 1.40, pushing the nabla_2 node unphysically deep (>2 bar).
    'dlog_P_bot': ([0.30,  1.40], r'$\Delta\log P_{\rm bot}$'),
    'dlog_P_top': ([0.20,  1.60], r'$\Delta\log P_{\rm top}$'),
    # v3.4.5: [C/H] replaces [Fe/H]/[Z/H] as the metallicity free parameter.
    # Internally: [Z/H] = [C/H] − log10(C/O / 0.5495) is passed to the table.
    # Prior range [-2, 3] is conservative; at solar C/O it maps 1:1 to [Z/H].
    'C_H':   ([-1.5, 1.5], r'$[{\rm C/H}]$'),
    'C/O':   ([0.1,  1.5], r'C/O'),         # table grid: 0.1 to 1.6; solar ≈ 0.55
    'log_12CO_13CO': ([0.5, 3.0], r'log $^{12}$CO/$^{13}$CO'),  # solar = log10(70) ≈ 1.845; range covers sub-solar to very high
...
# v3.4: instrument_res estimated per-night from wavelength solution
