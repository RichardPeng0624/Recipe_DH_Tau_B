...
class Parameters:
    ...
    def __call__(self, cube, ndim=None, nparams=None):
        ...
        for i, key_i in enumerate(self.param_keys):
            ...
            # nabla_RCE must appear before nabla_0-4 in free_params for this to work
            ...
            if key_i in ['nabla_3','nabla_4']:  # radiative zone: lower bound 0.00
...
class pRT_spectrum:
    ...
    def make_pt(self):
        ...
        # 6 pressure nodes from deep (10² bar) to shallow (10⁻⁵ bar) — Eq. 11
        # v3.4.5: nabla_4 used as the single radiative-zone gradient (matches equa_chem_v3.4.5)
        log_P_nodes = np.array([
             ...
             log_P_RCE -   dlog_P_top,        # just above RCE, single radiative node
            -5.0,                             # P_5 = 10⁻⁵ bar (fixed top)
        ])
...
free_params = {
    ...
    'nabla_2':    ([0.04, 0.34],  r'$\nabla_{T,2}$'),
    'nabla_3':    ([0.00, 0.34],  r'$\nabla_{T,3}$'),
    # nabla_4: radiative zone gradient reaching the model top (matches equa_chem_v3.4.5)
    'nabla_4':    ([0.00, 0.34],  r'$\nabla_{T,4}$'),
