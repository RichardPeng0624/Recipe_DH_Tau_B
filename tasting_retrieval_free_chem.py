...
class LogLikelihood:
    ...
    def __init__(self,retrieval_object,scale_flux=True,scale_err=True,alpha=2,N_phi=1):
        ...
        self.testing = retrieval_object.testing
        self.per_chip_scaling = retrieval_object.per_chip_scaling

        self.K2166 = retrieval_object.K2166 

    def __call__(self, m_flux, Cov):
        self.ln_L   = 0.0
        self.chi2_0 = 0.0
        self.m_flux_phi = np.zeros_like(self.d_flux)  # Initialize scaled model flux

        N_d = self.d_mask.sum() # Number of (valid) data points
        #d_flux = np.where(self.d_mask, self.d_flux, np.nan) # data flux with original shape targeting the isfinite mask, masked with NaN for the excluded values
        
        ref_mask = self.d_mask.flatten() if len(self.d_mask.shape) == 3 else self.d_mask  # flatten if 3D
        #m_flux = np.where(ref_mask, m_flux, np.nan) # model flux with original shape, masked with NaN for the excluded values

        wave_reshape = self.data_wave.reshape(1,-1) if len(self.data_wave.shape) == 3 else self.data_wave
        ref_wave = wave_reshape.flatten()  # flatten if 3D



        #m_flux_masked = np.where(self.d_mask, m_flux, np.nan) # model flux with original shape, masked with NaN

        # Check if we want per-chip scaling (when normalize_flux == False)
        if hasattr(getattr(self, 'retrieval_object', None), 'normalize_flux'):
            normalize_flux = self.retrieval_object.normalize_flux
        else:
            # fallback: try to infer from d_flux shape
            normalize_flux = False

        # Try to infer shape for per-chip scaling
  
        # If d_flux is 3D and normalize_flux is False, use per-chip scaling
        if self.testing == False:
            if not normalize_flux and len(self.d_flux.shape) == 3 and self.per_chip_scaling:
                
                n_orders, n_dets, n_pixels = self.d_flux.shape
                if n_pixels != 2048:
                    raise ValueError("Error: d_flux shape not recognized for per-chip scaling!")
                '''
                if not (self.d_mask.shape == self.d_flux.shape and m_flux.shape == self.d_flux.shape):
                    raise ValueError("Error: m_flux or d_mask shape not compatible for per-chip scaling!")
                '''
        else:
            # In testing mode, per_chip_scaling is forced to False because of 1D spectra
            self.per_chip_scaling = False
        

        if self.scale_flux and self.per_chip_scaling:
            # Per-chip scaling: fit phi for each (order, detector)
            phi = np.ones((n_orders, n_dets))
            m_flux_phi = np.zeros_like(self.d_flux)
            phi_list = []
            chi2_0 = 0.0
            ...
            for i in range(n_orders):
                for j in range(n_dets):
                    ...
                    #set up clipper for model flux since it is a 1D array when generated from pRT
                    
                    wl_range = self.data_wave[i,j] # for K2166 setting, orders are reversed

                    #check if waveelength range aligns with reference wave
                    wl_min = np.min(ref_wave)
                    wl_max = np.max(ref_wave)

                    if wl_range[0] < wl_min or wl_range[-1] > wl_max:
                        raise ValueError(f"Error: wavelength range {wl_range} of order {i}, detector {j} is out of bounds of reference wavelength grid {wl_min}-{wl_max}!")

                    mask_ij = self.d_mask[i, j]
                    #Note: It might not be necessary to mask model flux with a certain infinite mask, since the model flux is generated based on the valid data wavelength grid
                    #Then it should be finite at the same pixels as the data flux
                    #mask_model_ij = ref_mask[ (ref_wave >= wl_range[0]) & (ref_wave <= wl_range[-1]) ]

                    m_flux_ij = m_flux[ (ref_wave >= wl_range[0]) & (ref_wave <= wl_range[-1]) ]                 
                    d_ij = self.d_flux[i, j][mask_ij]
                    m_ij = m_flux_ij [mask_ij]
                    cov_ij = Cov.cov[i, j][mask_ij]

                    #check the shapes
                    if len(m_ij) != len(d_ij):
                        raise ValueError(f"Error: model flux {len(m_ij)} not compatible with data mask shape {len(d_ij)} for order {i}, detector {j}!")
                    ...
                    phi_ij = self.get_flux_scaling(d_ij, m_ij, cov_ij)[1]
                    phi[i, j] = phi_ij
                    m_flux_phi[i, j][mask_ij] = m_ij * phi_ij


                    residuals_ij = (d_ij - m_ij * phi_ij) # Compute the residuals
                    inv_cov_0_residuals_ij = 1/cov_ij * residuals_ij # Solve for inv_cov_0_residuals_phi. Assuming diagonal covariance
                    chi2_0 += np.dot(residuals_ij.T, inv_cov_0_residuals_ij) # Accumulate chi^2_0
            ...
            self.phi = phi
            self.m_flux_phi = m_flux_phi


        elif self.scale_flux:
            # Default: single scaling factor
            #flatten d_flux 

            print ('Updating data_flux, data_wave and Cov to 1D arrays for scaling...')
            d_flux_reshape = self.d_flux.reshape(1, -1) if len(self.d_flux.shape) == 3 else self.d_flux
            d_flux_flat = d_flux_reshape.flatten()
            
            self.d_flux = d_flux_flat

            m_flux_phi_reshape = m_flux.reshape(1, -1) if len(m_flux.shape) == 3 else m_flux
            m_flux_flat = m_flux_phi_reshape.flatten()
            self.m_flux_phi = m_flux_flat

            d_wave_reshape = self.data_wave.reshape(1, -1) if len(self.data_wave.shape) == 3 else self.data_wave
            d_wave_flat = d_wave_reshape.flatten()
            self.data_wave = d_wave_flat

            d_mask_reshape = self.d_mask.reshape(1, -1) if len(self.d_mask.shape) == 3 else self.d_mask
            d_mask_flat = d_mask_reshape.flatten()
            self.d_mask = d_mask_flat
            
            print ('original Cov shape:', Cov.cov.shape )
            cov_reshape = Cov.cov.reshape(1, -1) if len(Cov.cov.shape) == 3 else Cov.cov
            cov_flat = cov_reshape.flatten()
            print ('flattened Cov shape:', cov_flat.shape )
            Cov.cov = cov_flat  # update covariance to 1D array with only valid datapoints
            print ('updated Cov shape:', Cov.cov.shape )

            self.m_flux_phi[self.d_mask], self.phi = self.get_flux_scaling(self.d_flux[self.d_mask], m_flux[self.d_mask], Cov.cov[self.d_mask])

            residuals_phi = (self.d_flux - self.m_flux_phi)
            #inv_cov_0_residuals_phi = Cov.solve(residuals_phi[self.d_mask])
            inv_cov_0_residuals_phi = 1/Cov.cov[self.d_mask] * residuals_phi[self.d_mask]  # Solve for inv_cov_0_residuals_phi. Assuming diagonal covariance

            # Compute chi^2_0, meaning: before any scaling of uncertainties
            chi2_0 = np.dot(residuals_phi[self.d_mask].T, inv_cov_0_residuals_phi)

        else:
            self.m_flux_phi = m_flux
        ...
        # Compute logdet(M^T * cov^-1 * M), meaing: after any scaling of model flux 
        logdet_MT_inv_cov_0_M = 0

        if self.scale_flux:
            if self.per_chip_scaling:
                # For per-chip, sum over all chips
                MT_inv_cov_0_M = 0
                for i in range(n_orders):
                    for j in range(n_dets):

                        mask_ij = self.d_mask[i, j]
                        mask_model_ij = ref_mask[ (ref_wave >= self.data_wave[i,j][0]) & (ref_wave <= self.data_wave[i,j][-1]) ]

                        m_flux_ij = m_flux[ (ref_wave >= self.data_wave[i,j][0]) & (ref_wave <= self.data_wave[i,j][-1]) ]
                        
                        m_ij = m_flux_ij[mask_ij]
                        inv_cov_0_M = (1/Cov.cov[i, j][mask_ij]) * m_ij # Solve for inv_cov_0_M. Assuming diagonal covariance
                        MT_inv_cov_0_M += np.dot(m_ij.T, inv_cov_0_M)

                if MT_inv_cov_0_M > 0:
                    logdet_MT_inv_cov_0_M = np.log(MT_inv_cov_0_M)
            else:
                #inv_cov_0_M = Cov.solve(m_flux[self.d_mask], Cov.cov[self.d_mask])  # Solve for inv_cov_0_M. Assuming diagonal covariance
                inv_cov_0_M = (1/Cov.cov[self.d_mask]) * m_flux[self.d_mask]  # Solve for inv_cov_0_M. Assuming diagonal covariance
                MT_inv_cov_0_M = np.dot(m_flux[self.d_mask].T, inv_cov_0_M)
                logdet_MT_inv_cov_0_M = np.log(MT_inv_cov_0_M)
        else:
            logdet_MT_inv_cov_0_M = 0  # No scaling of model flux

        # Scale uncertainties if requested
        if self.scale_err:
            self.s2 = self.get_err_scaling(chi2_0, N_d)

        # Compute logdet(cov_0), meaning: after any scaling of uncertainties
    ...
    def get_flux_scaling(self, d_flux, m_flux, cov):
        """
        Solve for linear scaling parameter phi:
        (M^T * cov^-1 * M) * phi = M^T * cov^-1 * d
        """
        # Ensure covariance is inverted for scaling
        inv_cov = 1 / cov  # Element-wise inversion (assuming diagonal covariance)

        # Compute left-hand side and right-hand side
        lhs = np.dot(m_flux.T, inv_cov * m_flux)  # Left-hand side
        rhs = np.dot(m_flux.T, inv_cov * d_flux)  # Right-hand side

        # Avoid division by near-zero values
        phi = rhs / lhs if lhs > 1e-10 else 1.0

        # Return scaled model flux and scaling factor
        return m_flux * phi, phi

    def get_err_scaling(self, chi_squared_scaled, N):
        s2 = np.sqrt(chi_squared_scaled / N)  # Correct reduced chi-squared scaling
...
class pRT_spectrum:
    ...
    def make_spectrum(self):
        ...
        if self.normalize_flux == True:
            flux /= np.nanmedian(flux) # normalize flux -- pRT3 codes--
            print ("Normalized model flux by its median value.")
...
class Retrieval:
    ...
    def __init__(self,parameters,N_live_points,evidence_tolerance, target, testing=True, normalize_flux=False, ppid=None, per_chip_scaling=False):
        ...
        self.Cov = Covariance(err=err_masked) # simple diagonal covariance matrix with masked data, shape (orders, detectors, pixels) or 1D 
    ...
    def get_params_and_spectrum(self): 
        ...
        #reshape model flux to (orders, detectors, pixels) if needed
        if len(self.data_wave.shape) ==3: # if data is in (orders, detectors, pixels)
            model_flux_reshape = np.zeros_like(self.data_wave)
            for order in range(self.target.n_orders):
                for det in range(self.target.n_dets):
                    clip_range = self.data_wave[order,det]
                    clip_mask = (self.data_wave[order] >= clip_range[0]) & (self.data_wave[order] <= clip_range[-1])
                    model_flux_reshape[order, det][clip_mask] = self.model_flux[(self.target.n_dets*order):(self.target.n_dets*order + self.target.n_dets)]


        self.model_flux_scaled = np.dot(model_flux_reshape, self.LogLike.phi)

        #calculate the final error bars after scaling
        if len(self.data_err.shape) ==3: # if data is in (orders, detectors, pixels)
            s2_reshape = np.zeros_like(self.data_err)
            for order in range(self.target.n_orders):
                for det in range(self.target.n_dets):
                    clip_range = self.data_wave[order,det]
                    clip_mask = (self.data_wave[order] >= clip_range[0]) & (self.data_wave[order] <= clip_range[-1])
                    s2_reshape[order, det][clip_mask] = self.LogLike.s2
       
        self.data_err_scaled = np.dot(self.data_err, s2_reshape)
...
spectra_AB = np.load('/data2/peng/extracted_spectra_combined_sigmaclipper.npy') #(3, 5, 2048)
spectra_AB_err = np.load('/data2/peng/extracted_spectra_combined_err_sigmaclipper.npy') #(3, 5, 2048)
...
free_params = {'rv': ([0,30], r'$v_{\rm rad}$'), # km/s - uniform prior
                'vsini': ([0,10], r'$v$ sin$i$'), # km/s
                'log_g':({'type': 'gaussian', 'mu': 3.5, 'sigma': 0.5}, r'log $g$'),
                'T0' : ([2000,5000], r'$T_0$'), # bottom of the atmosphere (hotter)
                'T1' : ([100,3000], r'$T_1$'),
                'T2' : ([100,3000], r'$T_2$'),
                'T3' : ([100,2000], r'$T_3$'),
                'T4' : ([100,1000], r'$T_4$'), # top of atmosphere (cooler)
...
T=Target(wl=wave_reordered, fl=spectra_AB_reordered, err=spectra_AB_err_reordered, name='dh_tau_b')
...
retrieval=Retrieval(parameters=parameters, 
                    ...
                    target=T, 
                    testing=False, 
                    normalize_flux=False, per_chip_scaling=False)
...
np.save(f'/data2/peng/retrievals/{retrieval.job_id}_N{retrieval.N_live_points}_ev{retrieval.evidence_tolerance}/retrieval_model_wave.npy', wave_reordered)
...
model_flux_bin = savgol_filter(model_flux_scaled[mask_model], 51, 2)  # window size 51, polynomial order 2
...
for det in range(3):
    for order in range(5):
        ...
        if retrieval.testing==True:
            ...
            model_flux_cut = model_flux_bin[(retrieval.data_wave[mask_model]>=wave_border.min()) & (retrieval.data_wave[mask_model]<=wave_border.max())]

            ax[det][order].scatter(wave_cut_data, data_flux_cut/np.nanmedian(data_flux_cut), label='data', s=4, color='darkgray')
            ax[det][order].scatter(wave_cut_model, model_flux_cut/np.nanmedian(model_flux_cut), label='model', s=4, color='red')
            ...
            wave_cut_data = retrieval.data_wave[order][det][mask]
            wave_cut_model = retrieval.data_wave[order][det][mask_model]

            data_flux_cut = data_flux_bin[(retrieval.data_wave[order][det][mask]>=wave_border.min()) & (retrieval.data_wave[order][det][mask]<=wave_border.max())]
            model_flux_cut = model_flux_bin[(retrieval.data_wave[order][det][mask_model]>=wave_border.min()) & (retrieval.data_wave[order][det][mask_model]<=wave_border.max())]
