...
class pRT_spectrum:
    ...
    def equilibrium_chemistry(self, params):
        ...
        # 12CO/13CO isotopologue ratio (retrieved parameter; solar = 70, log10(70) ≈ 1.85)
        ratio_12CO_13CO = 10**params['log_12CO_13CO']

        for pRT_name, simple_name in zip(self.species, simple_names):
            if pRT_name == '13C-16O':
                # 13CO absent from table; derive from total CO using retrieved isotopologue ratio
                # mf_13CO = mf_CO * (m_13CO / m_12CO) / ratio_12CO_13CO
                mass_fractions['13C-16O'] = (
                    mass_fractions_eq['CO'] * (29.002355 / 28.009999) / ratio_12CO_13CO
                )
...
# -------------------------------------------------------#
# Species loaded (K-band): H2O, 12CO, 13CO, CH4, NH3, H2S, HCN, CO2, FeH
# (edit eq_species_names in get_species() to change)

constant_params = {
    'chemistry': 'equilibrium',   # activates equilibrium code path
}

# free parameters: key, prior range, mathtext label (for plotting)
