...
class pRT_spectrum:
    ...
    def equilibrium_chemistry(self, params):
        ...
        for pRT_name, simple_name in zip(self.species, simple_names):
            if pRT_name == '13C-16O':
                # 13CO absent from table; derive from total CO with solar 12C/13C = 70
                # mf_13CO = mf_CO * (m_13CO / m_12CO) / 70  (factor ≈ 0.0148)
                mass_fractions['13C-16O'] = (
                    mass_fractions_eq['CO'] * (29.002355 / 28.009999) / 70.0
                )
    ...
    def _make_raw_spectrum(self):
        """Compute pRT forward model, apply RV shift and rotational broadening.

        This step is the computational bottleneck (pRT radiative transfer).
        The result is cached on the instance so that make_spectrum() can be
        called twice per PMN_lnL evaluation (once per night) without repeating
        the expensive pRT calculation.

        Returns
        -------
        waves_even : 1-D array, nm
            Uniformly-spaced wavelength grid covering the full pRT range.
        spec : 1-D array
            Flux after RV shift and rotational broadening, on waves_even.
        """
    ...
    def make_spectrum(self, data_wave=None, out_res=None):
        """Generate the model spectrum for one night.

        The expensive pRT + RV + rotational-broadening step is cached on the
        instance, so calling this method twice per PMN_lnL evaluation (once for
        each night) only runs pRT once.

        Parameters
        ----------
        data_wave : 1-D array, nm
            Wavelength grid of the night's data to interpolate the model onto.
            Defaults to self.data_wave (night-1 grid stored in the instance).
        out_res   : int or float
            Instrument resolving power R = λ/Δλ for this night's LSF broadening.
            Defaults to self.spectral_resolution (100 000 for CRIRES+).
        ...
        if out_res is None:
            out_res = self.spectral_resolution

        # Compute raw spectrum (pRT + RV + rot. broad.) once per instance
        if not hasattr(self, '_raw_waves_even') or self._raw_waves_even is None:
            self._raw_waves_even, self._raw_spec = self._make_raw_spectrum()

        waves_even = self._raw_waves_even
        spec       = self._raw_spec

        if np.all(spec == spec[0]):
            print("WARNING: Flux is constant! Possible issue with the atmospheric model.")

        # Instrumental LSF broadening: from pRT native resolution to instrument resolution.
        # in_res  = pRT native R ≈ 1e6 / lbl_opacity_sampling  (333 333 for sampling=3)
        # out_res = CRIRES+ resolving power R ≈ 100 000 (per-night value passed in)
...
class Retrieval:
    ...
    def __init__(self, parameters, N_live_points, evidence_tolerance, targets,
                 ...
                 instrument_res=100_000):
        """
        Parameters
        ----------
        targets        : list of Target objects — one element for single-night,
                         two elements for joint two-night retrieval.
        instrument_res : CRIRES+ resolving power R = λ/Δλ used for LSF broadening.
                         Scalar → same value for all nights.
                         List of ints → one value per night (len must match targets).
                         Default: 100 000.
        ...
        # --- normalise instrument_res to a per-night list ---
        ...
        self.pressure = np.logspace(-5,1,self.n_atm_layers)  # 10⁻⁵–10^2 bar; matches DG anchor points (Picos+2025) but reduce the bottom pressure from 100 to 10 bar since CRIRES+ is not sensitive to deeper layers and it speeds up the pRT calculation
    ...
    def get_species(self, param_dict):
        ...
        if param_dict.get('chemistry', 'free') == 'equilibrium':
            ...
            # not by log_* free params. Adjust this list to match your opacity
            # library holdings and the molecules relevant to your wavelength range.
            eq_species_names = ['H2O', '12CO', '13CO', 'CH4', 'NH3', 'H2S', 'HCN', 'CO2', 'FeH']
            self.chem_species = [f'eq_{s}' for s in eq_species_names]
            return [species_info.loc[name, 'pRT_name'] for name in eq_species_names
                    if name in species_info.index]

        # Free chemistry: scan for log_* params (existing logic, unchanged)
        self.chem_species = []
        for par in param_dict:
            if par.startswith('log_') and par not in {'log_g', 'log_P_RCE', 'log_Pquench'}:
                self.chem_species.append(par)
        # pRT3 name overrides: species_info.csv has pRT2/ambiguous names for some species.
        # When multiple line lists exist in the opacity library, pRT3 prompts interactively
        # (crashes under MPI). Override with fully-qualified names here.
        _prt3_name_override = {
            '12C-1H4':     '12C-1H4__MM',       # CH4: MM (ExoMol) chosen over HITRAN2024
            'CO2_main_iso':'12C-16O2__HITEMP',   # CO2: HITEMP chosen (3 line lists available)
            'FeH_main_iso':'56Fe-1H',            # FeH: pRT2 name → pRT3 isotopologue
            'HCN':         '1H-12C-14N',         # HCN: pRT2 name → pRT3 isotopologue
            'H2S':         '1H2-32S',            # H2S: pRT2 name → pRT3 isotopologue
            'NH3':         '14N-1H3',            # NH3: pRT2 name → pRT3 isotopologue
        }
        species = []
    ...
    def PMN_lnL(self, cube=None, ndim=None, nparams=None):
        # One pRT_spectrum instance per call; caches the expensive radiative-transfer
        # step so that make_spectrum() can be called twice without re-running pRT.
        self.model_object = pRT_spectrum(self)

        # Night 1 — broaden to night-1 instrument resolution, interpolate, compare
        flux1 = self.model_object.make_spectrum(
            data_wave=self.data_wave, out_res=self.instrument_res[0]
        )
        ln_L = self.LogLike(flux1, self.Cov)

        # Night 2 — re-use cached raw spectrum, broaden to night-2 resolution
        if self.two_night_mode:
            flux2 = self.model_object.make_spectrum(
                data_wave=self.data_wave2, out_res=self.instrument_res[1]
            )
    ...
    def get_params_and_spectrum(self):
        ...
        # Create model spectra — one per night (shares the cached raw pRT spectrum)
        self.model_object = pRT_spectrum(self)
...
# -----------------------------------------------------------------------
# Helper: estimate instrument resolving power from a wavelength solution
# -----------------------------------------------------------------------
def estimate_spectral_resolution(wave_3d):
    """Estimate the median instrument resolving power R = λ/Δλ_pixel.

    Computed independently for each order/detector to avoid discontinuities
    at chip boundaries when the array is multi-order.

    Parameters
    ----------
    wave_3d : ndarray, shape (n_orders, n_dets, n_pixels), wavelengths in nm

    Returns
    -------
    R_median : float
        Median resolving power across all (order, detector) pairs.

    Notes
    -----
    R = λ / Δλ_pixel is the per-pixel resolving power.  For CRIRES+ K2166
    the pixel velocity sampling is ≈ c/R ≈ 3 km/s per pixel (i.e. one pixel
    per resolution element), so this estimator directly returns the instrument
    resolving power without any oversampling correction.
    """
    R_chips = []
    for order in range(wave_3d.shape[0]):
        for det in range(wave_3d.shape[1]):
            ...
            if np.sum(finite) < 2:
                continue
            w_valid = w[finite]
            dw      = np.diff(w_valid)              # Δλ per pixel (nm)
            wc      = 0.5 * (w_valid[1:] + w_valid[:-1])  # pixel-centre λ (nm)
            R_chip  = wc / np.abs(dw)               # R = λ / Δλ per pixel
            R_chips.append(float(np.nanmedian(R_chip)))
    R_median = float(np.median(R_chips))
    print(f'  Estimated resolving power R = {R_median:.0f}  '
          f'(median over {len(R_chips)} chips, '
...
# Returns (wave_flat_masked, flux_flat_masked, err_flat_masked, R_estimated)
# All spectrum arrays are 1-D with NaN/inf pixels removed (only valid kept).
# -----------------------------------------------------------------------
def _load_night(night, flux_file, err_file, normalize_method, wave_path=None, n_orders=5, n_dets=3):
    ...
    wave_path      : optional path to wavelength file; if None, uses default path {night}/cal/
    ...
    # Load wavelength solution: use provided path if given, otherwise use default
    if wave_path is not None:
        wave_file = f'{wave_path}'

        #check that the provided wave file has the expected shape (3, 5, 2048), must be fits or npy format
        if wave_file.endswith('.fits'):
            wave_hdu = fits.open(wave_file)
            wave     = np.array(wave_hdu[1].data)  # (3, 5, 2048)
        elif wave_file.endswith('.npy'):
            wave = np.load(wave_file)  # (3, 5, 2048)
        
        if wave.shape != (3, n_orders, 2048):
            raise ValueError(f'Provided wavelength file has shape {wave.shape}, expected (3, {n_orders}, 2048)')
    ...
    # Estimate resolving power from the wavelength solution (on 3D array, before flattening)
    print(f'  [{night}] Estimating resolving power from wavelength solution...')
...
wave_N1, flux_N1, err_N1, R_N1 = _load_night(
    ...
    wave_path=f'{workpath}{night1}/barycentric_wavelengths_night1.npy',  # use barycentric wavelengths for night 1 (legacy)
...
# -----------------------------------------------------------------------
# Night 2: 2023-01-01
# NOTE: uses _0101-suffixed files from cooking_subtraction_0101.ipynb
# -----------------------------------------------------------------------
night2 = '2023-01-01'
wave_N2, flux_N2, err_N2, R_N2 = _load_night(
    ...
    flux_file='extracted_spectra_combined_sigmaclipper.npy',
    err_file ='extracted_spectra_combined_err_sigmaclipper.npy',
    wave_path=f'{workpath}{night2}/barycentric_wavelengths_night2.npy',  # use barycentric wavelengths for night 2 (legacy)
    normalize_method=Normalize_method,
)
...
free_params = {#'rv': ([27, 37], r'$v_{\rm rad}$'), # km/s - uniform prior
                'rv': ({'type': 'gaussian', 'mu': 16.3, 'sigma': 0.5}, r'$v_{\rm rad}$'), # km/s - Gaussian prior based on literature value, barycentric RV
                'vsini': ([0,20], r'$v$ sin$i$'), # km/s
                #'vsini': ({'type': 'gaussian', 'mu': 9.6, 'sigma': 0.5}, r'$v$ sin$i$'), # km/s
                'epsilon': ([0.0, 1.0], r'$\epsilon_{\rm LD}$'), # limb-darkening coeff for rotational broadening; 0=uniform, 1=full LD
    ...
    'nabla_0':    ([0.04, 0.34],  r'$\nabla_{T,0}$'),          # at P_0=10^1 bar; constrained ≤ nabla_RCE
    ...
    'T_bottom':   ([2000, 5000],  r'$T_{\rm bot}$'),           # K at P_0=10^1 bar; old retrieval ~2300-2536 K
...
# }
# free_params = {
#     'rv':      ({'type': 'gaussian', 'mu': 32, 'sigma': 0.1}, r'$v_{\rm rad}$'),
...
#     'FeH':   ([-2.0, 3.0], r'[Fe/H]'),     # table grid: -2 to +3; solar = 0.0
#     'C/O':   ([0.1,  1.6], r'C/O'),         # table grid: 0.1 to 1.6; solar ≈ 0.55
...
# instrument_res: CRIRES+ K2166 nominal R = 100 000 (same for both nights)
retrieval = Retrieval(parameters=parameters,
                      ...
                      instrument_res=[R_N1, R_N2])        # estimated from per-night wavelength solutions
