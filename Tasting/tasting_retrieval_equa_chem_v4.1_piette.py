"""
tasting_retrieval_equa_chem_v4.1_piette.py
===========================================
Series 004PM-EQ — Equilibrium chemistry + Piette & Madhusudhan (2020) P-T profile

[003PM-EQ → 004PM-EQ: Guidebook v1.0 → v1.1]
  - log_g replaced by log_M + log_R as free parameters (Gaussian priors from
    Xuan+2024 Table 1: M = 12±4 MJup, R = 2.6±0.6 RJup).
  - log_g is derived internally: g = G*M/R² (cgs).
  - Motivation: retrieval 3078846 gave log_g = 4.25, ~6σ above evolutionary
    models (expected log_g ≈ 3.64 for 12 MJup at 2.6 RJup).
    Xuan+2024 §4.3.5 adopt the identical M+R prior approach.

Two-night joint retrieval for DH Tau B (CRIRES+ K2166).
Calls Guidebook_GAStronomy_Piette_v1.1 for all classes and RT machinery.

Data
----
Night 1: 2022-12-31
Night 2: 2023-01-01
Files  : extracted_spectra_combined_flux_cal.npy  (flux)
...
_spec = importlib.util.spec_from_file_location('Guidebook_GAStronomy_Piette_v1_1', _guidebook_path)
...
# ===========================================================================
# FREE PARAMETERS — Piette+2020 P-T + equilibrium chemistry (004PM-EQ)
# ===========================================================================

constant_params = {'chemistry': 'equilibrium'}

# Default equilibrium parameter set from Guidebook v1.1 (log_M, log_R,
# T_anchor + dT_1-7, C_H, C/O, log_12CO_13CO).  Modify priors here if needed.
# log_g is no longer a free parameter — derived from log_M and log_R.
...
# log_M and log_R priors are already set in make_free_params_equilibrium() from
# Xuan+2024 Table 1 (DH Tau b: M=12±4 MJup, R=2.6±0.6 RJup). Override here
# if needed for a different target.
