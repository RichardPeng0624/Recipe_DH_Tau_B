...
=========================================================================
Series 008PM-EQ-MEDNORM-ATOMOPAC — Equilibrium chemistry, cloud-free,
per-chip median normalisation (data AND model), Standard Gaussian
likelihood, GP correlated noise, Na+Ca opacity restored to the model
spectrum WITH ZERO NEW FREE PARAMETERS.

Purpose
-------
Direct sibling of 008PM-EQ-MEDNORM (job 3497146, this script's un-suffixed
v6.1 ancestor) -- identical data, normalisation, likelihood, and GP kernel.
The ONLY change is that this run's model spectrum includes Na and Ca
opacity via Guidebook v4.2.5, whereas 3497146 has neither (both species were
fully commented out of EQ_SPECIES_PRT3 on 2026-07-07).

This is deliberately NOT the same test as the 2026-07-07/08 A/B
(2968924/3160171 [with Na,Ca as free log-VMR params] vs. 3497146/3708914
[without]) -- that A/B tested removing Na/Ca as *sampled parameters* and
found it changes nothing (DlnZ < 1 nat, all shared params <1sigma). It never
isolated whether Na/Ca *opacity in the forward model* matters independent of
giving the sampler two extra dimensions. This run tests exactly that,
holding the parameter count fixed at 3497146's 19 free params:
  - Na: sourced directly from the pRT3 equilibrium-chemistry table, driven
    by the SAME C_H/C/O free params already in 3497146 -- verified live
    that 'Na' is a native table key (petitRADTRANS.chemistry.utils.
    _solar_elemental_abundances / PreCalculatedEquilibriumChemistryTable
    ().species, Lodders 2020). No log_Na parameter is read.
  - Ca: not in the table, so it is scaled directly from the same derived
    metallicity ZH using a fixed solar Ca/H ratio (Lodders 2020,
    LOG_CA_H_SOLAR = -5.73, read from petitRADTRANS's own solar-abundance
    table -- the same source already backing CO_SOLAR = 0.5495). No log_Ca
    parameter is read.
Full derivation: recording_recipe.md, 2026-07-20, "Investigation: can Na/Ca
opacity re-enter the model spectrum WITHOUT adding free parameters?" and the
Guidebook_GAStronomy_Piette_v4.2.5.py changelog at the top of that file.

If this run reproduces 3497146 to <1sigma on every shared parameter (as the
2026-07-07/08 free-parameter A/B did), that closes the Na/Ca question on
BOTH axes (parameter freedom AND opacity) for the paper's Methods section
reconciliation with 2027997 (which retains Na/Ca as free params). If it
does NOT reproduce 3497146, that is new information the earlier A/B could
not have surfaced, since it never put the opacity back in.

Everything below (data loading, likelihood, GP, plotting) is otherwise
IDENTICAL to tasting_retrieval_equa_chem_v6.1_piette_cloudfree_mednorm.py --
see that script's docstring for the full calibration-strategy rationale
(sidesteps SINFONI absolute flux calibration entirely).
...
N_points     = 600      # matches 3497146 (cloud-free sibling)
evidence_tol = 0.5
...
# ===========================================================================
# FREE PARAMETERS — 008PM-EQ-MEDNORM-ATOMOPAC
...
print('Expected count: 19 (same as 3497146 — Na/Ca add opacity, not dimensions)')
...
plt.title('P-T profile (Piette+2020 PCHIP) — 008PM-EQ-MEDNORM-ATOMOPAC')
