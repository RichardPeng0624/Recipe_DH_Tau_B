"""
Guidebook_GAStronomy_Piette_v4.0.py
=====================================
...
- log_R prior changed to uniform U(-0.1, 0.5) — data constrains R directly
  from the absolute flux level; evolutionary model prior no longer needed.
...
EQ_SPECIES_PRT3 = [
    ...
    '23Na',               # Na   — Kurucz (Allard+2019)
    # '48Ti',             # Ti   — commented out (log_Ti not a free param)
    '40Ca',               # Ca   — Kurucz (Castelli & Kurucz 2003)
]
...
def make_free_params_equilibrium():
    ...
    return {
        ...
        'log_R':    ([-0.1, 0.5],                                        r'$\log R/R_{\rm Jup}$'),
...
def make_free_params_free_chem():
    ...
    return {
        ...
        'epsilon':  ([0.0, 1.0],                                        r'$\epsilon_{\rm LD}$'),
        # v2.0: log_M Gaussian; log_R uniform (data constrains from absolute flux)
        'log_M':    ({'type': 'gaussian', 'mu': 1.079, 'sigma': 0.145}, r'$\log M/M_{\rm Jup}$'),
        'log_R':    ([-0.1, 0.5],                                        r'$\log R/R_{\rm Jup}$'),
        ...
        'dT_4':     ([  0,  500], r'$\Delta T_4$'),  # -0.7 → -1.0 bar (0.3 dex); Table C1: U(0,450)
        'dT_5':     ([ 50,  700], r'$\Delta T_5$'),  # -1.0 → -1.5 bar (0.5 dex); Table C1: U(50,650)
        'dT_6':     ([100,  800], r'$\Delta T_6$'),  # -1.5 → -3.0 bar (1.5 dex); Table C1: U(200,900)
        'dT_7':     ([100,  800], r'$\Delta T_7$'),  # -3.0 → -5.0 bar (2.0 dex); Table C1: U(100,700)
